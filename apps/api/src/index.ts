...
import { AgentDirectory, CANONICAL_TAGS, expandQuery } from "@tsugu/discover";
...
const client = new TsuguClient({ rpcUrls: [RPC] });
...
app.get("/agents/:name", async (req, res) => {
  try {
    ...
    if (!agent) return res.status(404).json({ error: `${req.params.name}@tsugu not found` });
    res.json(agent);
  } catch (e) {
    res.status(502).json({ error: (e as Error).message });
  }
...
app.listen(PORT, () => {
  ...
  console.log(`tsugu discovery API → http://localhost:${PORT} (chain ${client.chainId}, ttl ${TTL_MS}ms)`);
});
