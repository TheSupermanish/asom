...
| **Discovery** — advertise capabilities; find agents by what they do | ✅ | `CapabilityRegistry` · `@tsugu/discover` · web `Discover` |
...
```bash
tsugu somnia                              # show Somnia's base agents (registry on mainnet / constants on testnet)
tsugu ai classify "is 2+2=4?" --allow yes,no
tsugu ai judge 7                          # advisory accept/reject on a submitted task
```
...
tsugu intentionally does **not** hard-depend on Somnia's curated, mainnet-only AgentRegistry
(empty bytecode on Shannon). `TsuguClient.resolveSomniaAgents()` reads it on mainnet and falls
