...
export class TsuguClient {
  ...
  constructor(opts: TsuguClientOptions = {}) {
    ...
    if (opts.privateKey) {
      ...
      if (!acct) {
        throw new Error("tsugu: a walletClient without a bound account requires an explicit `account`");
      }
      this.account = acct;
  ...
  private llmAddr(): Address {
    ...
    if (!a) throw new Error("tsugu: no LlmAgent deployed for this chain; pass addresses.llmAgent");
    return a;
  }

  private parseAddr(): Address {
    const a = this.addresses.parseAgent;
    if (!a) throw new Error("tsugu: no ParseAgent deployed for this chain; pass addresses.parseAgent");
    return a;
  }
  ...
  private async dispatchAi(
    ...
    if (!ev) throw new Error(`tsugu: ${functionName} landed but RequestDispatched log was missing`);
  ...
  async waitForAiResult(
    ...
    for (;;) {
      ...
      if (Date.now() > deadline) {
        throw new Error(`tsugu: AI request ${requestId} (${kind}) did not finalize within ${timeoutMs}ms`);
      }
  ...
  async somniaRequestDeposit(): Promise<bigint> {
    ...
    if (!platform) throw new Error(`tsugu: no Somnia Agents platform known for chain ${this.chainId}`);
    return this.publicClient.readContract({
      address: platform,
      abi: [
        {
