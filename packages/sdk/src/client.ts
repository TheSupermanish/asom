...
export class TsuguClient {
  ...
  async createAgent(
    ...
    if (!this.walletClient || !this.account) {
      throw new Error("tsugu: createAgent requires a privateKey");
    }
  ...
  async send(to: Address, stt: string): Promise<Hash> {
    if (!this.walletClient || !this.account) {
      throw new Error("tsugu: send requires a privateKey");
    }
    ...
    const receipt = await this.publicClient.waitForTransactionReceipt({ hash });
    if (receipt.status === "reverted") {
      throw new Error(`tsugu: send to ${to} reverted (tx ${hash})`);
    }
    return hash;
  }
