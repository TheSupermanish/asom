...
import type { AsomClient } from "@asom/sdk";
...
/**
 * Minimal slice of AsomClient that the keychain needs — lets tests pass a stub
...
export function ownerKeyFor(
  ...
  if (!seed) {
    if (agentOwner.toLowerCase() !== operatorAddress.toLowerCase()) {
      throw new Error(`${agentName}@asom is owned by ${agentOwner}, not your key ${operatorAddress} — you don't control it`);
    }
    return operatorKey;
  }
  const idx = indexOfOwner(seed, agentOwner, max);
  if (idx === null) {
    throw new Error(`${agentName}@asom is owned by ${agentOwner}, which isn't derived from your seed — you don't control it`);
  }
