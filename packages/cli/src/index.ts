...
// --- Somnia base AI agents + tsugu AI compute --------------------------------
...
async function runAi(
  label_: string,
  dispatch: (c: TsuguClient) => Promise<{ requestId: bigint; txHash: Hex }>,
...
ai
  ...
  .action(async (taskId: string, opts: { allow: string }) => {
    ...
    console.log(muted(`  → If 'accept': ${accent(`tsugu task approve ${taskId}`)}.  If 'reject': ${accent(`tsugu task refund ${taskId}`)}.`));
  });
