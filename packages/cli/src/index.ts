...
function printAgent(c: TsuguClient, agent: Agent, balanceWei?: bigint) {
  console.log("");
  console.log(`  ${pc.bold(pc.bgMagenta(pc.white(` ${agent.name}@tsugu `)))}  ${muted("self-sovereign agent")}`);
  console.log("");
...
program
  ...
  .action(async (name: string, opts: { seed: string; gas: string }) => {
    ...
    await assertFunded(c, parseFloat(opts.seed) + parseFloat(opts.gas) + 0.05, `create ${name}@tsugu`);
    ...
    try {
      ...
      console.log(ok(`  ✨ ${pc.bold(name + "@tsugu")} is alive and owns itself.`));
      ...
      if (gasTx) {
        ...
        console.log(muted(`  note: fund ${name}'s owner key to let it act — tsugu fund ${name}`));
      }
      console.log("");
    } catch (err) {
...
program
  ...
  .action(async (name: string) => {
    ...
    console.log(
      free ? ok(`  ✓ ${pc.bold(name + "@tsugu")} is available`) : bad(`  ✗ ${name}@tsugu is taken`),
    );
  });
...
program
  ...
  .action(async (name: string, opts: { gas: string; wallet: string }) => {
    ...
    await assertFunded(c, total + 0.02, `fund ${name}@tsugu`);
