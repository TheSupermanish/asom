...
async function getKey(): Promise<`0x${string}`> {
  ...
  if (hasKeystore()) {
    ...
    if (envPw) console.error(muted("  ⚠ using ASOM_PASSWORD from env — for automation only (it's visible to child processes)."));
...
program
  ...
  .action(async (opts: { import?: boolean }) => {
    ...
    console.log("");
    console.log(ok(`  ✓ Wallet ${mode === "new" ? "created" : "imported"} and encrypted → ${ASOM_HOME}/keystore.json`));
    ...
    if (mode === "new") {
      ...
      console.log(muted(`  Back it up any time with: asom key export`));
    }
    console.log(muted("  Your key is encrypted on this machine. asom never sends or stores it anywhere."));
  });
