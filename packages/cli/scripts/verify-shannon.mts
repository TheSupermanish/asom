...
const operator = new AsomClient({ privateKey: PK, rpcUrls: [RPC] });
...
async function main() {
  log(`\n■ asom live verification — ${NAME}@asom on Shannon (chain ${operator.chainId})`);
  ...
  log(`\n[1] created ${NAME}@asom  ${explorer(agent.txHash)}`);
  ...
  const owner = new AsomClient({ privateKey: ownerKey, rpcUrls: [RPC] });
  ...
  log(`\n[4] transferred ${NAME}@asom → ${newOwnerAddr}  ${explorer(t)}`);
  ...
  log(`  name=${NAME}@asom tokenId=${agent.tokenId} wallet=${agent.account}`);
