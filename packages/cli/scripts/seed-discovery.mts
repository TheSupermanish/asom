...
const c = new AsomClient({ privateKey: PK, rpcUrls: [RPC] });
...
async function main() {
  ...
  try {
    ...
    console.log(`reusing ${name}@asom (token ${tokenId})`);
  } catch {
    const agent = await c.createAgent(name, { seedStt: "0" }); // owned by operator
    tokenId = agent.tokenId;
    console.log(`created ${name}@asom (token ${tokenId}) ${c.explorer("tx", agent.txHash)}`);
  }
