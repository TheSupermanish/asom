...
// store.ts reads ASOM_HOME (via keystore.ts) at import time — set it, then import.
const home = mkdtempSync(join(tmpdir(), "asom-store-"));
let store: typeof import("../src/store.js");

beforeAll(async () => {
  process.env.ASOM_HOME = home;
...
describe("store (local agent records)", () => {
  ...
  it("lists saved agents and SKIPS a corrupt file without throwing", () => {
    ...
    // A hand-edited / partially-written file must not crash `asom ls`.
